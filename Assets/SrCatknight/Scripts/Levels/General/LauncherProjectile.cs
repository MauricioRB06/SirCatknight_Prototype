...
namespace SrCatknight.Scripts.Levels.General
{
    ...
    public class LauncherProjectile : MonoBehaviour
    {
        ...
        private int _orientationProjectile;
        private Vector2 _projectileLimit;
        ...
        private void Awake()
        {
            ...
            {
                _projectileSpeed = GetComponentInParent<LauncherObject>().ProjectileSpeed;
                _orientationProjectile = GetComponentInParent<LauncherObject>().LauncherRotation;
                _projectileLimit = GetComponentInParent<LauncherObject>().projectileLimit.transform.position;
        ...
        private void Update()
        {
            ...
            switch (_orientationProjectile)
            {
                case -1:
                {
                    transform.Translate(Vector2.left * (_projectileSpeed * Time.deltaTime));
                
                    if (!(Vector2.Distance(transform.position, _projectileLimit) < 0.1f)) return;
                    _projectileAnimator.SetTrigger(Crash);
                    break;
                }
                case 0:
                {
