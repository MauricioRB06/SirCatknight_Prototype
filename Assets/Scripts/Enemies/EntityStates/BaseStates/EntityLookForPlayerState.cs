...
namespace Enemies.EntityStates.BaseStates
{
    public class EntityLookForPlayerState : EntityState
    {
        ...
        private readonly DataEntityLookForPlayer dataEntityLookForPlayer;
        ...
        {
            this.dataEntityLookForPlayer = dataEntityLookForPlayer;
        }
        ...
        public override void LogicUpdate()
        {
            ...
            else if(Time.time >= LastTurnTime + dataEntityLookForPlayer.TimeBetweenTurns && !IsAllTurnsDone)
            {
                Core.Movement.Flip();
            ...
            if(AmountOfTurnsDone >= dataEntityLookForPlayer.AmountOfTurns)
            {
                IsAllTurnsDone = true;
            }
            
            // It is checked if the time for the last turn is completed.
            if(Time.time >= LastTurnTime + dataEntityLookForPlayer.TimeBetweenTurns && IsAllTurnsDone)
            {
